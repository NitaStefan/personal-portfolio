...
const ProjectImages = ({ images }: { images: string[] }) => {
  ...
  const { zoomed, projectImage, selectImage } = context;

  return (
    <div
      className={cn(
        "order-1 flex h-30 gap-1 transition-transform duration-400 ease-out sm:h-28 sm:gap-2 lg:order-2 lg:h-35 lg:w-110",
        zoomed && "translate-x-50 translate-y-14 lg:w-128",
      )}
    >
      <ArrowWrapper
        ...
        return (
          <button
            ...
            className={cn(
              ...
              isActive && "grow-2 text-sm font-medium opacity-85 sm:text-base",
            )}
          >
